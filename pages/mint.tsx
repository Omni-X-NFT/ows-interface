...
const mint: NextPage = () => {
  ...
  const [network, setNetwork] = useState<string>("137");
    ...
    if (addresses[chainId]) {
      try {
        ...
        const tokenContract = new ethers.Contract(
          addresses[`${Number(chainId).toString(10)}`].address,
          AdvancedONT.abi,
          signer
        );
  ...
  const mint = async (): Promise<void> => {
    ...
    try {
      ...
      if (saleFlag && publicmintFlag) {
        ...
        mintResult = await tokenContract.publicMint(mintNum, {
          value: ethers.utils.parseEther(
            (addresses[chainId].price * mintNum).toString()
          ),
        });
        ...
        if (Number(currentBalance) + mintNum > 5) {
          errorToast(
            "You have already minted " +
              String(Number(currentBalance)) +
              " gregs \n" +
              "Can't mint more than 5 gregs in private sale"
          );
          setIsMinting(false);
        } else {
          mintResult = await tokenContract.mint(mintNum, merkleProof, {
            value: ethers.utils.parseEther(
              (addresses[chainId].price * mintNum).toString()
            ),
          });
      ...
      if (e["code"] == 4001) {
        ...
        if (
          Number(currentBalance) / Math.pow(10, 18) <
          addresses[chainId].price * mintNum
        ) {
          errorToast("There is not enough money to mint nft");
        } else {
          errorToast(
            "your address is not whitelisted on " + addresses[chainId].name
          );
        }
      }
      setIsMinting(false);
  ...
  const sendNFT = async (): Promise<void> => {
    ...
    try {
      ...
      const estimateFee = await tokenContract.estimateSendFee(
      ...
      destination_contract.on("Transfer", (from, to, tokenID) => {
        if (to == account) {
          toast.success(
            `${addresses[chainId].name} sent greg#${tokenID} to ${addresses[toChain].name}`,
            {
              position: toast.POSITION.TOP_RIGHT,
              autoClose: 3000,
              transition: Slide,
      ...
      <>
        {chainIds.map(function (chain, idx) {
          if (chain.chainId == network) {
            return (
              <option key={idx} value={chain.chainId} selected>
                {chain.name}
              </option>
            );
          } else {
            return (
              <option key={idx} value={chain.chainId}>
                {chain.name}
              </option>
            );
          }
    ...
    if (provider?.on) {
      ...
      const handleChainChanged = (chainId: any) => {
        if (isContainChains(parseInt(chainId, 16))) {
          ...
          errorToast(
            "The current network is not supported, please change the network"
          );
          switchNetwork();
          setNetwork("1");
        }
    ...
    const calculateFee = async (): Promise<void> => {
      try {
        if (transferNFT) {
          ...
          const fee: any = await tokenContract.estimateSendFee(
            addresses[toChain].chainId,
            account,
            transferNFT,
            false,
          ...
          setEstimateFee(
            "Estimate Fee :" +
              ((Number(fee[0]) / Math.pow(10, 18)) * 1.1).toFixed(10) +
              addresses[chainId].unit
          );
        } else {
          setEstimateFee("");
        }
        ...
        <div className={mintstyles.container}>
          ...
          <div className={mintstyles.mintDesc}>
            ...
            <div className={mintstyles.mintDescSec}>
              <p>
                greg is our genesis collection that represents our community and
                technological breakthroughs
              </p>
              <p>
                mint greg below from any chain you wish and transfer him to any
                other chain using the “Transfer” box below
              </p>
            ...
            <div className={mintstyles.mintDataGrid}>
              <div className={mintstyles.mintDataWrap}>
                <h5>MINTED</h5>
                <span>
                  {nextTokenId - substrateIndex}/
                  {totalNFTCount - substrateIndex}
                </span>
              </div>
              ...
              <div className={mintstyles.mintDataWrap}>
                ...
                <span>
                  ...
                  <Image
                    src={
                      chainId
                        ? addresses[`${Number(chainId)}`].imageSVG
                        : PolygonImageSVG
                    }
                    width={29.84}
                    height={25.46}
              ...
              <div className={mintstyles.mintDataWrap}>
                ...
                <div className={mintstyles.counterWrap}>
                  <button onClick={() => decrease()}>
                    <Image src={MinusSign} alt="minus"></Image>
                  </button>
                  <span>{mintNum}</span>
                  <button onClick={() => increase()}>
                    <Image src={PlusSign} alt="plus"></Image>
                  </button>
                </div>
              </div>
            </div>
              ...
              >
                <div className={selectstyles.chainIcon}>
                  <Image
                    ...
                    src={
                      chainId
                        ? addresses[`${Number(network)}`].image
                        : PolygonImage
                    }
                    width={29.84}
                    height={25.46}
